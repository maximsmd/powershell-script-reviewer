...
- **Computed-but-unused values.** Trace every value built by real work — an API call, a filter, a lookup — forward to confirm the logic that follows actually consumes it, rather than assuming it does because it looks load-bearing. A script that queries Graph to build a "stale profiles" list and then deletes everything under a path unconditionally, never once referencing that list, isn't doing targeted cleanup — it's doing "delete everything," with an unrelated identity dependency (and its auth, throttling, and failure surface) bolted on for logic that doesn't affect behavior. This is a correctness finding, not a style nit: the script's actual behavior has silently diverged from what its own code claims to do.
